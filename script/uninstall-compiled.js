...
class AirUninstaller {
  ...
  async run() {
    try {
      ...
      const confirm = await this.ui.confirm(`Are you sure you want to uninstall Air (${this.config.name})?`, false);
      if (!confirm) {
...
2. Remove npm packages:`);
  ...
  async stopProcesses() {
    try {
      ...
      try {
        if (isWindows()) {
          execSync3(`taskkill /F /IM node.exe /FI "WINDOWTITLE eq *air*"`, { stdio: "ignore" });
          stopped = true;
        } else {
          try {
            execSync3(`pkill -f "air.*main\\.(ts|js)"`, { stdio: "ignore" });
            stopped = true;
          } catch {}
        }
      } catch {}
  ...
  async removeService() {
    ...
    try {
      ...
      if (isWindows()) {
        ...
        let found = false;
        try {
          execSync3(`systemctl --user status air-${this.config.name}`, { stdio: "ignore" });
          found = true;
        } catch {}
        if (!found && hasSudo()) {
          try {
            execSync3(`sudo systemctl status air-${this.config.name}`, { stdio: "ignore" });
            found = true;
          } catch {}
        }
        if (!found) {
  ...
  async cleanPidFiles() {
    try {
      ...
      const files = fs3.readdirSync(this.config.root);
      files.forEach((file) => {
        if (file.startsWith(".air") && file.endsWith(".pid")) {
          fs3.unlinkSync(path3.join(this.config.root, file));
          cleaned++;
        }
      });
      if (cleaned > 0) {
  ...
  async removeCronJobs() {
    ...
    try {
      let currentCron = "";
      try {
        currentCron = execSync3("crontab -l", { encoding: "utf8" });
      } catch {
        return { success: true, message: "No crontab found" };
      }
      const lines = currentCron.split(`
`).filter((line) => !line.includes(this.config.root) && !line.includes("air") && !line.includes(this.config.name));
      if (lines.length !== currentCron.split(`
`).length) {
        const newCron = lines.join(`
`);
        const tmpFile = path3.join("/tmp", `cron-${Date.now()}`);
        fs3.writeFileSync(tmpFile, newCron);
        execSync3(`crontab "${tmpFile}"`, { stdio: "ignore" });
  ...
  async removeSSL() {
    try {
      let removed = 0;
      const sslDir = path3.join(this.config.root, "ssl");
      if (fs3.existsSync(sslDir)) {
        fs3.rmSync(sslDir, { recursive: true });
        removed++;
      }
      const userSSLDir = path3.join(process.env.HOME || "", ".local", "share", "air", "ssl");
      if (fs3.existsSync(userSSLDir)) {
        fs3.rmSync(userSSLDir, { recursive: true });
        removed++;
      }
      if (removed > 0) {
...
var uninstaller = new AirUninstaller;
