...
const getConfigPath = (cliArg = null, rootPath = null) => {
    if (cliArg)
        return path.resolve(cliArg);
    if (process.env.AIR_CONFIG)
        return path.resolve(process.env.AIR_CONFIG);
    if (process.env.AIR_CONFIG_PATH)
        return path.resolve(process.env.AIR_CONFIG_PATH);
    
    // Check XDG config directory
    const xdgConfig = process.env.XDG_CONFIG_HOME;
    if (xdgConfig) {
        const xdgPath = path.join(xdgConfig, 'air', 'config.json');
    ...
    const homeConfig = path.join(process.env.HOME || '~', '.config/air/config.json');
