...
export const IP_SERVICES = {
    dns: (process.env.AIR_DNS_RESOLVERS || "resolver1.opendns.com:myip.opendns.com,1.1.1.1:whoami.cloudflare").split(",").map(entry => {
        const [resolver, hostname] = entry.split(":")
        return { resolver: resolver!, hostname: hostname! }
    }),

    http: (process.env.AIR_HTTP_RESOLVERS || "https://api.ipify.org:text,https://icanhazip.com:text").split(",").map(entry => {
