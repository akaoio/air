...
export function createLogo(): string {
    ...
    return logoLines
        .map(line => `${colors.primary}${colors.bold}${line}${colors.reset}`)
        .join('\n')
}
...
export function createMenu(options: MenuOption[]): string {
    ...
    options.forEach(option => {
        const statusIndicator = option.status === 'modified' ? ` ${colors.warning}●${colors.reset}` :
                               option.status === 'disabled' ? ` ${colors.muted}(disabled)${colors.reset}` : ''
...
export function createFooter(commands: FooterCommand[], width?: number): string {
    ...
    if (terminal.isMobile) {
        ...
        const commandStrings = commands.map(cmd => 
            `${colors.accent}${colors.bold}[${cmd.key}]${colors.reset} ${colors.muted}${cmd.action}${colors.reset}`
        )
