    ...
    if (loading && !status) {
        return (
            <Box flexDirection="column" alignItems="center" justifyContent="center" minHeight={10}>
                <ModernHeader 
                    title="System Status" 
                    subtitle="Distributed P2P Database Monitor"
                    showLogo={true}
                />
                
                <Box marginY={2}>
                    <LoadingSpinner 
                        text="Scanning system status..." 
                        type="bouncingBall"
        ...
        return (
            <Box flexDirection="column" alignItems="center">
                <ModernHeader 
                    ...
                    subtitle="Unable to retrieve system information"
                    gradient="fire"
                    showLogo={false}
                />
                ...
                <StatusCard 
                    ...
                    value={error}
                    details={[
                        "Check if Air is properly configured",
                        "Verify file system permissions",
                        "Run 'air-install' to setup Air"
                    ]}
                />
                
                <ModernFooter commands={[
                    { key: 'R', action: 'Retry' }
                ]} />
            </Box>
        )
        ...
        <Box flexDirection="column">
            ...
            <Box justifyContent="space-between" marginY={1}>
                <StatusCard 
                    ...
                    details={status.process.running ? [
                ...
                <StatusCard 
                    ...
                    details={[
                        `Port ${status.port.number}: ${status.port.inUse ? 'In use' : 'Available'}`,
                        status.port.process ? `By: ${status.port.process.name}` : 'Ready to bind'
                    ]}
                />
            </Box>
            ...
            <Box justifyContent="space-between" marginY={1}>
                <StatusCard 
                    ...
                    value={status.ssl.configured ? 
                        (status.ssl.valid ? `SSL Active` : 'SSL Issues') : 
                        'SSL Not Configured'}
                    details={status.ssl.configured ? [
                        status.ssl.expiry ? `Expires: ${status.ssl.expiry}` : 'Checking expiry...',
                        'HTTPS enabled'
                    ] : [
                        'HTTP only',
                        'Run installer to setup SSL'
                    ]}
                />
                ...
                <StatusCard 
                    ...
                    details={status.ddns.configured ? [
            ...
            <Box justifyContent="space-between" marginY={1}>
                <StatusCard 
                    ...
                    details={status.peers.length > 0 ? [
                        'Distributed network active',
                        'Real-time synchronization'
                    ] : [
                        'Standalone mode',
                        'Add peers for clustering'
                    ]}
                />
            ...
            <ProgressBar 
                ...
                label={`🏥 System Health: ${getHealthStatus(healthScore)}`}
            ...
            <ModernFooter commands={[
                ...
                ...(options.watch ? [{ key: 'W', action: `Watching (${options.interval}s)` }] : [])
            ]} />
        </Box>
    )
}
