...
export function detectAirMode(): AirModeConfig {
    ...
    const isSuperPeer = fs.existsSync(path.join(cwd, "package.json")) && fs.existsSync(path.join(cwd, "src", "main.ts")) && JSON.parse(fs.readFileSync(path.join(cwd, "package.json"), "utf8")).name === "@akaoio/air"
...
export function ensureDirectories(config: AirModeConfig): void {
    const dirs = [path.dirname(config.configPath), config.dataPath, config.statePath, config.cachePath]
