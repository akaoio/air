...
async function runTests() {
    ...
    await test("Configuration read/write", async () => {
        ...
        const configManager = peer.configManager;
        configManager.configFile = path.join(testDir, "air.json");
        const success = await peer.write(config);
    ...
    await test("Network module imports", async () => {
        const { default: network } = await import("../src/network.js");
