...
export const detectionMocks = {
    existingConfig: {
        existsSync: jest.fn((filePath: string) => filePath.endsWith('air.json')),
...
export const pathMocks = {
    ...
    resolve: jest.fn((pathStr: string) => pathStr.startsWith('/') ? pathStr : `/current/${pathStr}`),
...
export function createInstallerMockContext(overrides: any = {}) {
    return {
...
export const installerTestUtils = {
    ...
    validateInstallerContext: (context: any) => {
    ...
    validateSystemInfo: (systemInfo: any) => {
    ...
    createTempOptions: (overrides: any = {}) => ({
        name: 'temp-installer',
        env: 'development',
        port: 8765,
        ...overrides
    ...
    simulatePrivileges: (isRoot: boolean, hasSudo: boolean) => {
        return {
            process: {
            ...
            execSync: jest.fn((command: string) => {
                if (command.includes('sudo -n true')) {
                    if (!hasSudo) throw new Error('sudo: password required')
                    return ''
                }
                return 'success'
            })
