...
import type { UpdaterOptions } from '../../src/Updater/constructor.js'
import type { GitResult } from '../../src/Updater/git.js'
import type { PackagesResult } from '../../src/Updater/packages.js'
import type { RestartResult } from '../../src/Updater/restart.js'
import { airConfigMocks } from './installerMocks.js'

// Updater options mocks
export const updaterOptionMocks = {
    default: {} as UpdaterOptions,
    basic: {
        autoRestart: false,
...
export const lockfileMocks = {
    bunRecentlyModified: {
        existsSync: jest.fn((filePath: string) => filePath.includes('bun.lockb')),
        statSync: jest.fn(() => ({
...
export const restartMocks = {
    linux: {
        ...
        systemdUser: {
            execSync: jest.fn((command: string) => {
                if (command.includes('systemctl --user restart')) return ''
                if (command.includes('systemctl --user is-active')) return 'active'
                if (command.includes('sudo -n true')) throw new Error('sudo password required')
                return ''
            })
        },
        systemdSystem: {
            execSync: jest.fn((command: string) => {
                if (command.includes('systemctl --user restart')) throw new Error('user service not found')
                if (command.includes('sudo systemctl restart')) return ''
        ...
        noSystemd: {
            execSync: jest.fn((command: string) => {
                if (command.includes('systemctl')) throw new Error('systemctl not found')
                if (command.includes('sudo -n true')) throw new Error('sudo not available')
                return ''
            })
        }
    },
    mac: {
        launchctl: {
            execSync: jest.fn((command: string) => {
                if (command.includes('launchctl stop')) return ''
                if (command.includes('launchctl start')) return ''
                return ''
            })
        },
        launchctlFails: {
            execSync: jest.fn((command: string) => {
                if (command.includes('launchctl')) throw new Error('service not found')
                return ''
            })
        }
    },
...
export const resultMocks = {
    git: {
        success: { success: true, message: 'Updated from repository', details: '3 files changed' } as GitResult,
        upToDate: { success: true, message: 'Already up to date' } as GitResult,
        notRepo: { success: false, message: 'Not a git repository' } as GitResult,
        failed: { success: false, message: 'Git update failed', details: 'Connection refused' } as GitResult
    },
    packages: {
        npmSuccess: { success: true, message: '3 packages updated' } as PackagesResult,
        npmUpToDate: { success: true, message: 'All packages up to date' } as PackagesResult,
        npmWithFix: { success: true, message: 'Packages updated & vulnerabilities fixed' } as PackagesResult,
        bunSuccess: { success: true, message: 'Packages updated (Bun)' } as PackagesResult,
        bunUpToDate: { success: true, message: 'All packages up to date (Bun)' } as PackagesResult,
        failed: { success: false, message: 'Package update failed', details: 'npm update failed' } as PackagesResult
    },
    restart: {
        success: { success: true, message: 'Process restarted' } as RestartResult,
        systemdUser: { success: true, message: 'User service restarted' } as RestartResult,
        systemdSystem: { success: true, message: 'System service restarted' } as RestartResult,
        launchd: { success: true, message: 'launchd service restarted' } as RestartResult,
        windows: { success: true, message: 'Will restart on next login' } as RestartResult,
        notRunning: { success: false, message: 'Process not running' } as RestartResult,
        failed: { success: false, message: 'Service restart failed' } as RestartResult
    }
}

// Create mock context for Updater testing
export function createUpdaterMockContext(overrides: any = {}) {
    return {
...
export const updaterTestUtils = {
    ...
    mockSystemEnvironment: (runtime: 'node' | 'bun', platform: 'linux' | 'windows' | 'mac') => {
        return {
            isBun: runtime === 'bun',
    ...
    validateGitResult: (result: GitResult) => {
    ...
    validatePackagesResult: (result: PackagesResult) => {
        expect(result).toEqual(expect.objectContaining({
            success: expect.any(Boolean),
            message: expect.any(String)
        }))
    ...
    validateRestartResult: (result: RestartResult) => {
        expect(result).toEqual(expect.objectContaining({
            success: expect.any(Boolean),
            message: expect.any(String)
        }))
    ...
    countCommands: (mockExecSync: jest.MockedFunction<any>) => {
    ...
    validateUpdateStatus: (updater: any) => {
