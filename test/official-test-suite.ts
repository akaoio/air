...
${colors.reset}`)
     ...
     * Run a test command
     */
    private async runTest(name: string, command: string, options: {
        timeout?: number
    ...
    async testPackageScripts() {
        ...
        const scriptsToTest = [
            ...
            // Type checking (allow failures for now)
            {
                ...
                timeout: 30000,
                checkOutput: (output: string) => true // Always pass for now
            },
        ...
        for (const script of scriptsToTest) {
            ...
            await this.runTest(
                `package.json script: ${script.name}`,
                command,
                script
            )
        }
    ...
    async testUIComponents() {
        ...
        await this.runTest(
            'Viewport detection',
            'npx tsx script/test-viewport.ts 2>&1 | head -50',
            {
        ...
        await this.runTest(
            'BeautifulConsole rendering',
            'npx tsx -e "import { createHeader } from \'./src/UI/BeautifulConsole.js\'; console.log(createHeader(\'Test\', \'Subtitle\'))"',
            {
    ...
    async testCoreModules() {
        ...
        await this.runTest(
            'Config module',
            'npx tsx -e "import { Config } from \'./src/Config/index.js\'; const c = new Config(\'/tmp/test-air.json\'); console.log(\'Config OK\')"',
            {
        ...
        await this.runTest(
            'Process module',
            'npx tsx -e "import { Process } from \'./src/Process/index.js\'; const p = new Process({ root: \'/tmp\', name: \'test\' }); console.log(\'Process OK\')"',
            {
        ...
        await this.runTest(
            'DDNS module',
            'npx tsx -e "import { DDNS } from \'./src/DDNS/index.js\'; console.log(\'DDNS OK\')"',
            {
        ...
        await this.runTest(
            'Path module',
            'npx tsx -e "import { getPaths } from \'./src/paths.js\'; const p = getPaths(); console.log(p.root ? \'Paths OK\' : \'Paths FAIL\')"',
            {
    ...
    async testScriptCommands() {
        ...
        for (const script of scripts) {
            ...
            await this.runTest(
                `${scriptName} --help`,
                `npx tsx ${script} --help 2>&1 | head -20`,
                {
                    checkOutput: (output) => 
            ...
            if (scriptName === 'status-modern') {
                await this.runTest(
                    `${scriptName} --non-interactive`,
                    `npx tsx ${script} --non-interactive 2>&1`,
                    {
    ...
    async testEnvironmentVariables() {
        ...
        for (const env of environments) {
            await this.runTest(
                `AIR_ENV=${env}`,
                'npx tsx -e "console.log(process.env.AIR_ENV || \'not set\')"',
                {
                    env: { AIR_ENV: env },
        ...
        await this.runTest(
            'Termux detection',
            'npx tsx -e "import viewport from \'./src/UI/Viewport.js\'; console.log(viewport.isTermux ? \'Termux\' : \'Not Termux\')"',
            {
